...
class BaseDescent:
    ...
    def calc_gradient(self, x: np.ndarray, y: np.ndarray) -> np.ndarray:
        ...
        if self.loss_function == LossFunction.MSE:
            ...
            return 1 / len(y) * x.T @ np.tanh(x @ self.w - y)

        else:
            raise AttributeError('The loss function used is not supported')
...
class StochasticDescent(VanillaGradientDescent):
    ...
    def calc_gradient(self, x: np.ndarray, y: np.ndarray) -> np.ndarray:
        ...
        if self.loss_function == LossFunction.MSE:
            return 2 / x_batch.shape[0] * x_batch.T.dot(errors)

        elif self.loss_function == LossFunction.LogCosh:
            return np.dot(x_batch.T, np.tanh(errors)) / len(x_batch)
